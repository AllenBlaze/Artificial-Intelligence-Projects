...
class DecisionTree: #nom de la class à changer
	...
	def train(self, train, train_labels): #vous pouvez rajouter d'autres attribus au besoin
		...
		self.build_tree()
		self.nbClass = np.unique(train_labels)

		self.predicted_label = np.array([])
		for idx in train:
			self.predicted_label = np.append(self.predicted_label, self._predict(self.root, idx))
		train_matrix = self.get_confusion_matrix(train_labels, self.predicted_label)
		print("---TRAIN---")
		print("Matrix de confusion")
		print (train_matrix)
		train_accuracy = self.get_accuracy(train_matrix)
	...
	def get_confusion_matrix(self, real_label, predicted_label):
		y = len(self.nbClass)
		confusion_matrix = np.zeros((y, y), dtype=int)
		i = 0
		while i < len(real_label):
			A = int(real_label[i])

			B = int(predicted_label[i])
	...
	def predict(self, exemple, label):
		...
		"""
		prediction = self._predict(self.root,exemple)
		if prediction != label :
                        return false
		if prediction == label :
                        return true
	...
	def test(self, test, test_labels):
		...
		for idx in test:
                        self.predicted_label = np.append(self.predicted_label, self._predict(self.root,idx))
		test_matrix = self.get_confusion_matrix(test_labels, self.predicted_label)
		print("---TEST---")
		print("Matrix de confusion")
		print (test_matrix)
		test_accuracy = self.get_accuracy(test_matrix)
